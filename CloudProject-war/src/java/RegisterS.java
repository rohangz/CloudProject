...
public class RegisterS extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("</head>");
            out.println("<body><cnter>");
            out.println("<h1>Servlet RegisterS at " + request.getContextPath() + "</h1>");
            out.println(request.getParameter("usr")+request.getParameter("pw")+request.getParameter("enr")+
            request.getParameter("full"));
            String user=request.getParameter("usr");
            String pass=request.getParameter("pw");
            String enrol=request.getParameter("enr");
            String full=request.getParameter("full");
            int x=s.register(user, pass, enrol, full);
            out.println(x);
